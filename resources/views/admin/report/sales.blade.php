...
<head>
    ...
    <title>Sales Report - My Batik Admin</title>
            ...
            <div class="flex items-center space-x-3">
                <div x-data="{ dropdownOpen: false }" class="relative">
                    ...
                    <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-50" x-cloak>
                        ...
                        <form method="POST" action="{{ route('logout') }} M">@csrf<a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="block px-4 py-2 text-sm font-semibold text-red-600 hover:text-red-800 transition">Logout</a></form>
                    </div>
                </div>
            </div>
        </div>
        ...
        <main class="bg-white rounded-2xl shadow-sm p-6 md:p-8">
            <div class="flex flex-col md:flex-row gap-8">
                <aside class="w-full md:w-1/4">
                    ...
                    <ul class="space-y-2">
                        <li><a href="{{ route('admin.reports.sales') }}" class="block px-4 py-2 rounded-md bg-blue-100 text-blue-700 font-bold">Monthly Sales</a></li>
                        <li><a href="{{ route('admin.reports.promo') }}" class="block px-4 py-2 rounded-md hover:bg-gray-100 font-semibold text-gray-600">Promo Code Usage</a></li>
